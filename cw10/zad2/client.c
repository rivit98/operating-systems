...
void init_socket(const char *type, const char *dest){
    if(type[0] == 'l'){ //local
        ...
        binded_socked_fd = socket(AF_UNIX, SOCK_DGRAM, 0);
        ...
        bind(binded_socked_fd, (struct sockaddr*)&binded_sockaddr, sizeof(struct sockaddr_un));
    }else{ //network
        is_local = false;
        char ip_s[32] = {};
        char port_s[16] = {};
...
int main(int argc, char **argv){
    ...
    if(is_local){
        char a[3];
        sprintf(a, "%d", REGISTER_CLIENT);
        a[1] = '\0';
        sendto(server_socket_fd, a, MAX_MESSAGE_LEN, 0, (const struct sockaddr *) &serveraddr, sizeof(struct sockaddr));
    }else{
    ...
    while(1){
        if(is_local){
            if(recv(binded_socked_fd, buffer, MAX_MESSAGE_LEN, 0) == 0){
                continue;
            }
        }else{
