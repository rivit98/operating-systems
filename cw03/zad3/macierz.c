...
unsigned int worker_cb(List *list, time_t deadline, int proc_time, int max_memory, MODE mode);
...
unsigned int worker_cb(List *list, time_t deadline, int proc_time, int max_memory, MODE mode){
	set_limits(proc_time, max_memory);
	unsigned int ret = 0;
	int col_to_mult;
...
int main(int argc, char **argv) {
	if (argc != 7) {
		printf("Usage:\n");
		printf("macierz lista_macierzy proc_num max_time method proc_time memory_MB\n");
		return 1;
	}
	...
	op.mode = atoi(argv[4]);
	op.proc_time = atoi(argv[5]);
	op.max_memory = atoi(argv[6]);
	...
	for(int i = 0; i < op.proc_num; i++){
		pid_t child = fork();
		if(child == 0){
			int mul = worker_cb(&m_list, cur_time + op.max_time, op.proc_time, op.max_memory, op.mode);
