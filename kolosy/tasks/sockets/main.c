...
int uname(int s, char *name){
    ...
    strncpy(sockaddrUn.sun_path, name, sizeof(sockaddrUn.sun_path));

    return bind(s, (const struct sockaddr*) &sockaddrUn, SUN_LEN(&sockaddrUn));
}
...
int uconnect(int s, char *name){
    ...
    return connect(s, (const struct sockaddr*) &sockaddrUn, SUN_LEN(&sockaddrUn));
}
